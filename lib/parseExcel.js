...
const FLEET_REGISTRY={RCI:"rc",CCL:"carnival",NCL:"ncl"};
...
const formatShip=(raw)=>{const s=String(raw||"").trim();let m;m=s.match(/^RCI(.+?)\s*OTS$/i);if(m)return"RCI "+m[1].trim()+" OTS";m=s.match(/^CCL(.+)$/i);if(m)return"Carnival "+m[1].trim();m=s.match(/^NCL(.+)$/i);if(m)return"Norwegian "+m[1].trim();return s||null;};
...
const mondayKey=dt=>{const d=new Date(dt);const dow=(d.getDay()+6)%7;d.setDate(d.getDate()-dow);return d.toISOString().slice(0,10);};
const estNow=()=>{const u=new Date();return new Date(u.getTime()-5*3600000);};
const lastCompletedWeekMonday=()=>{const n=estNow();const dow=(n.getUTCDay()+6)%7;const thisMon=new Date(Date.UTC(n.getUTCFullYear(),n.getUTCMonth(),n.getUTCDate()-dow));const lastMon=new Date(thisMon.getTime()-7*86400000);return lastMon.toISOString().slice(0,10);};
