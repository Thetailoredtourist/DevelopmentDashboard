...
function buildVoyages(recs){
  ...
  const voys=Object.values(byVoy).map(v=>{
    ...
    return{month:v.month,week:v.week,date:v.recs[0].vstart||"",vdate:v.recs[0].vstart,vd:v.recs[0].vdays||0,ship:formatShip((v.recs[0].vid||"").slice(0,-13).trim()),
...
function build(data,ships){
  ...
  const weekly=voys.map(v=>({m:MS[v.month],v:v.week||0,date:v.date,vd:v.vd,vs:Math.round(v.pctVoy*100),sp:v.sp,sd:Math.round(v.sales),gap:v.gap,aur:v.aur,tr:Math.round(v.trans),u:Math.round(v.units),upt:v.upt,atv:v.atv,ship:v.ship,nr:v.nr,nb:v.nb}));
  const ms=MONTHS_FULL.map(m=>monthly[MS[m]]?Math.round(monthly[MS[m]].sd):null);// full year: data past June is no longer dropped
  const ship=latestShip(recs);
  let shipMove=null;{const wv=weekly.filter(w=>w&&w.ship&&!w.nb);for(let x=wv.length-1;x>0;x--){if(wv[x].ship!==wv[x-1].ship){const prev=wv[x-1],cur=wv[x];const pe=new Date((prev.date||"")+"T00:00:00");if(!isNaN(pe.getTime())){pe.setDate(pe.getDate()+((prev.vd||1)-1));}const cs=new Date((cur.date||"")+"T00:00:00");const gapDays=(!isNaN(pe.getTime())&&!isNaN(cs.getTime()))?(cs.getTime()-pe.getTime())/86400000:999;// a true transfer: the new ship began within 28 days of the previous voyage ending; a longer gap is a vacation break, not a transfer
    if(gapDays<=28){shipMove={from:prev.ship,to:cur.ship,date:cur.date,prevEnd:new Date(pe.getTime()).toISOString().slice(0,10)};}break;}}}
  return{name:data.name,org:data.org,ship,shipMove,status,greyOut,module:"Unknown",mc:0,acc,mtd,wkAvg,traj,trajD,growth,tier,monthly,weekly,ms};
}
